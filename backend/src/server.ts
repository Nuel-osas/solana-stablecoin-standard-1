...
app.listen(PORT, () => {
  ...
  console.log(`  Stablecoin Mint: ${STABLECOIN_MINT_STR ?? "not set"}`);
  console.log(`  Operator: ${operatorKeypair ? operatorKeypair.publicKey.toBase58() : "not configured"}`);
  console.log(`  Auth: ${API_KEY ? "enabled" : "disabled (set API_KEY to enable)"}`);
  console.log(`  Audit Log: ${AUDIT_LOG_PATH} (${auditLog.length} entries loaded)`);
  console.log(`  Webhooks: ${webhooks.length} registered`);
});
